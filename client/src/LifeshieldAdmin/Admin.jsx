    ...
    {hotdatam?.map((entry, index) => (
      <tr key={index} className="hover:bg-gray-50 text-[14px]">
        ...
        <td className="py-3 px-4 border-b">
          {
            entry.verify?<span> Allready  Verify This  </span>:<button className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded " onClick={()=>{handleVefy(entry._id);setTrue(true)}}>
            Verify
          </button>
          }
        </td>
